...
const updateFcmToken = async (req, res) => {
  try {
    ...
    const tokenDoc = { token: fcmToken, platform: platform || 'android', lastActive: new Date() };

    // Essai User collection d'abord
    ...
    if (user) {
      // Utilisateur client/admin — welcome notif si première connexion
      ...
      if (driver) {
        console.log(`✅ FCM token sauvegardé pour le chauffeur ${driver.name}`);
      } else {
        console.warn(`⚠️ updateFcmToken: aucun utilisateur trouvé pour id ${userId}`);
      }
    }
